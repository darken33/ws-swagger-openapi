...
=== Les premiers pas du Design First chez Generali

Nous avons poussé pour expérimenter le desgin d'API et la génération de code chez Generali, cependant nous ne voulions pas impacter l'ensemble du projet.
...
Il s'agit de l'étape suivante logique, une fois que la technique est suffisament éprouvée (cap  non encore franchit chez Generali)
